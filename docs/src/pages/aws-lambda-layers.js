...
function appVersion(response) {
  const rows = response.split('\n');
  let html = '';

  for (const row of rows) {
    if (row.includes('AppDefender Version')) {
      html = row;
      break;
    }
  }
...
function Versions() {
  ...
  fetch(customFields.AWS_LAYER_VERSIONS_URL)
    ...
    .then(response => {
      if (isClientSide()) {
        ...
        Array.from(document.getElementsByClassName('class-th-appversion')).forEach(e => e.innerHTML = appVersion(response));
      };
  });
        ...
        <div class="row">
          <div class="col col--6" style={{marginTop: '48px'}}>
            ...
            <table style={{marginTop: '24px'}}>
              <thead>
                <tr>
                  <th colSpan={3} style={{textAlign: 'left'}} class="class-th-appversion">AppDefender</th>
                </tr>
                <tr>
                  <th>Region</th>
                  <th>ARN for x86-64</th>
                  <th></th>
                </tr>
              </thead>
